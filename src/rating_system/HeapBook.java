...
public class HeapBook {
     ...
     */
    HeapBook(Record[] data,Linked_List_Book listBook)
    {
    ...
    HeapBook(Record[] data,Linked_List_Book listBook,int select)
    {
        this.select=select;
        this.listBook=listBook;
        heap=data.clone();
...
*/
    private void Max_heapify(int rootIndex)
    {
        ...
        while(this.left(root)<heap_size)
        {
            if(select!=0)
            {
                if((listBook.search(heap[this.left(root)].getBook()).getRating())>(listBook.search(heap[root].getBook()).getRating()))
                {
                    max=this.left(root);
                }
                else
                ...
                if((this.right(root)<heap_size)&&(listBook.search(heap[this.right(root)].getBook()).getRating()>listBook.search(heap[max].getBook()).getRating()))
                {
                    max=this.right(root);
                }
            }
            ...
            {
                if((listBook.search(heap[this.left(root)].getBook()).getRatingAlternative())>(listBook.search(heap[root].getBook()).getRatingAlternative()))
                {
                    max=this.left(root);
                }
                else
                ...
                if((listBook.search(heap[this.right(root)].getBook()).getRatingAlternative())>(listBook.search(heap[max].getBook()).getRatingAlternative()))
                {
                    max=this.right(root);
                }
            }
